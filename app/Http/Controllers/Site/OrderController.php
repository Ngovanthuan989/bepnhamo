...
class OrderController extends SiteController
{
   ...
   public function sendNew(Request $request){
       ...
       $orderId = $order->insertGetId([
           ...
           'total_price' => $request->input('total_payment'),
//           'method_payment' =>  $methodPayment,
           'cost_ship' => $request->input('ship_fee'),
...
//               'properties' => $orderItem->properties,
       ...
       $order = Order::with('orderDetails')->find($orderId);
        ...
        return redirect()->route('redirect_checkout', ['id' => $orderId]);
   }

    public function redirectCheckout($id){
        $orderId = $id;
...
//           DB::beginTransaction();
       ...
       $orderShips = OrderShip::get();
           ...
           // lấy ra chi phí ship
           $costShip = $request->input('cost_ship');
